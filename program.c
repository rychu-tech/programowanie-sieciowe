...
#define IP_POOL_SIZE 254 // Without server address and 0 address
...
void initialize_ip_pool(ip_address* pool) {
    for (int i = 2; i < IP_POOL_SIZE; i++) {
        pool[i-2].octets[0] = 192;
        pool[i-2].octets[1] = 168;
        pool[i-2].octets[2] = 1;
        pool[i-2].octets[3] = i;
...
int create_offer_packet(dhcp_packet *offer_packet, dhcp_packet *discovery_packet, ip_address* pool) {
...
int main() {
    ...
    while(1) {
        ...
        if (discovery_packet.options[2] == DHCP_DISCOVERY) {
            ...
            clean_ip_pool(pool);
            int offer_packet_size = create_offer_packet(&offer_packet, &discovery_packet, pool);
            // print_ip_pool(pool);
        ...
        if (discovery_packet.options[2] == DHCP_REQUEST) {
            ...
            int ack_packet_size = create_ack_packet(&ack_packet, &discovery_packet, requested_ip, pool);
